...
class NoticeModel(dbSession):
    def add_notice(self, obj: notice_add_interface):  # 管理员发布公告
        ...
        with self.get_db() as session:
            ...
            session.flush()
            session.commit()
            return obj_add.nt_id, obj_add.nt_gmt_create
    ...
    def update_notice(self, notice_update: notice_update_interface):
        with self.get_db() as session:
            timenow = datetime.datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S")
            session.query(Notice).filter(Notice.nt_id == notice_update.nt_id, Notice.nt_is_deleted == 0).update(
                {"nt_title": notice_update.nt_title, "nt_content": notice_update.nt_content,
    ...
    def get_notice_by_nt_id(self, nt_id):  # 根据nt_id查询notice的基本信息
        with self.get_db() as session:
            ...
            ses = session.query(Notice.nt_gmt_create, Notice.nt_gmt_modified, Notice.u_id, Notice.nt_title,
...
class UserNoticeModel(dbSession):
    def add_user_notice(self, nt_id: int, u_id: int):  # 添加已读记录
        obj = notice_user_add_interface(nt_id=nt_id, u_id=u_id)
        obj_dict = jsonable_encoder(obj)
    ...
    def judge_exist_by_u_n(self, u_id: int, nt_id: int):  # 根据u_id,nt_id查是否已读
        with self.get_db() as session:
            nu_is_read = session.query(UserNotice.nu_is_read).filter(
                UserNotice.nt_id == nt_id, UserNotice.u_id == u_id).first()
            session.commit()
